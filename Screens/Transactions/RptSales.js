...
export default function RptSales({ navigation, route }) {
    ...
    Sharing.isAvailableAsync().then((result) => {
      if (result) {
        FileSystem.downloadAsync(
          `https://musicstore.quickgst.in/App_Rtp/SalesRpt.aspx?tran_id=${invoice_id}`,
          FileSystem.cacheDirectory + `PakkaBill${invoice_id}.pdf`
        )
          .then(async ({ uri }) => {
            setURL(uri);
          })
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <Button
          ...
          onPress={() => {
            Print.printAsync({
              uri: `https://musicstore.quickgst.in/App_Rtp/SalesRpt.aspx?tran_id=${invoice_id}`,
            });
          }}
        >
          Print
