...
export default function RptPo({ navigation, route }) {
    ...
    Sharing.isAvailableAsync().then((result) => {
      if (result) {
        FileSystem.downloadAsync(
          `https://musicstore.quickgst.in/App_Rtp/PoRpt.aspx?tran_id=${tran_id}`,
          FileSystem.cacheDirectory + `PO${tran_id}.pdf`
        )
          .then(async ({ uri }) => {
            setURL(uri);
          })
    ...
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <Button
          ...
          onPress={() => {
            Print.printAsync({
              uri: `https://musicstore.quickgst.in/App_Rtp/PoRpt.aspx?tran_id=${tran_id}`,
            });
          }}
        >
          Print
