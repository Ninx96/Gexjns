...
export default function GandhiNgrLocationForm({ route, navigation }) {
  ...
  const Preview = () => {
    postData("Masters/PreviewGandhiNgrLocation", param).then((data) => {
      //console.log(data);
      setParam(data);
      setloading(false);
    });
 ...
 <Spinner
        ...
        <ScrollView>
          <View style={{ padding: 50 }}>
            <Headline style={{ alignSelf: "center", marginBottom: 6 }}>
              Gandhi Nagar Location Details
            </Headline>

            <TextInput
              style={styles.input}
              mode="outlined"
              label={"Gandhi Nagar Location"}
      ...
      <FAB
        ...
        icon="check"
        onPress={() => {
          postData("Masters/InsertGandhiNgrLocation", param).then((data) => {
            if (data.valid) {
