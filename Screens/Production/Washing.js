...
export default function Washing({ route, navigation }) {
  ...
  const Delete = (tran_id) => {
    setloading(true);
    postData("Production/DeleteProductionWashing", { tran_id: tran_id }).then((data) => {
      Refresh();
      setloading(false);
    });
  };
  ...
  const Hide = (tran_id, hide) => {
    ...
    setloading(true);
    postData("Production/UpdateWashingHide", _param).then((data) => {
      Refresh();
      setloading(false);
    });
  };
  ...
  const Refresh = () => {
    postData("Production/BrowseProductionWashing", param).then((data) => {
      // console.log(data);
      setGrid(data);
      setloading(false);
    });
  ...
  return (
    <>
      <ProductionHeader index={6} navigation={navigation} />
      <View style={{ flex: 1 }}>
        <Spinner
          visible={isloading}
          textContent={"Loading.."}
