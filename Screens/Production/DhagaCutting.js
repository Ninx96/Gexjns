...
export default function DhagaCutting({ route, navigation }) {
  ...
  const widthArr = [100, 100, 80, 150, 100, 100, 100, 100, 100, 100, 100, 100];
  ...
  const Delete = (tran_id) => {
    setloading(true);
    postData("Production/DeleteProductionDhagaCutting", { tran_id: tran_id }).then((data) => {
      Refresh();
      setloading(false);
    });
  };
  ...
  const Hide = (tran_id, hide) => {
    ...
    setloading(true);
    postData("Production/UpdateDhagaCuttingHide", _param).then((data) => {
      Refresh();
      setloading(false);
    });
  };
  ...
  const Refresh = () => {
    postData("Production/BrowseProductionDhagaCutting", param).then((data) => {
      // console.log(data);
      setGrid(data);
      setloading(false);
    });
  ...
  return (
    <>
      <ProductionHeader index={7} navigation={navigation} />
      <View style={{ flex: 1 }}>
        <Spinner
          visible={isloading}
          textContent={"Loading.."}
