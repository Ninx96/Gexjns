...
export default function KajButton({ route, navigation }) {
  ...
  const Delete = (tran_id) => {
    setloading(true);
    postData("Production/DeleteProductionKaj", { tran_id: tran_id }).then((data) => {
      Refresh();
      setloading(false);
    });
  };
  ...
  const Hide = (tran_id, hide) => {
    ...
    setloading(true);
    postData("Production/UpdateKajHide", _param).then((data) => {
      Refresh();
      setloading(false);
    });
  };
  ...
  const Refresh = () => {
    postData("Production/BrowseProductionkajbutton", param).then((data) => {
      // console.log(data);
      setGrid(data);
      setloading(false);
    });
